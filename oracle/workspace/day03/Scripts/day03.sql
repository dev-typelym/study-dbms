...
/*PLYAER 테이블에서 POSITION이 NULL인 선수 검색*/
SELECT * FROM PLAYER
WHERE "POSITION" IS NULL;
/*PLYAER 테이블에서 POSITION이 NULL인 선수를 '미정'으로 변경 후 검색*/
SELECT PLAYER_NAME, NVL("POSITION",'미정') FROM PLAYER;
/*PLYAER 테이블에서 NATION이 등록되어 있으면 '등록', 아니면 '미등록'으로 검색*/
...
SELECT PLAYER_NAME || '의 영어 이름은 ' || E_PLAYER_NAME || '이다.' AS 영어이름 FROM PLAYER;

/*PLAYER_NAME의 포지션은 POSITION입니다.*/
SELECT PLAYER_NAME || '의 포지션은 ' || NVL("POSITION", '미정') || '입니다.' AS "선수 포지션" FROM PLAYER;
...
SELECT POSITION, AVG(HEIGHT) AS "포지션별 평균 키", (SELECT AVG(HEIGHT)FROM PLAYER) AS "전체 평균 키"
FROM PLAYER
WHERE "POSITION" IS NOT NULL
GROUP BY "POSITION" 
...
SELECT T.TEAM_ID, PLAYER_NAME, TEL FROM TEAM T JOIN PLAYER P 
ON T.TEAM_ID = P.TEAM_ID AND PLAYER_NAME = '송종국';


/*JOBS 테이블에서 JOB_ID로 직원들의 JOB_TITLE, EMAIL, 성, 이름 검색*/
SELECT JOB_TITLE, e.EMAIL, e.FIRST_NAME, e.LAST_NAME FROM JOBS j  JOIN EMPLOYEES e  
...
SELECT j.JOB_TITLE, AVG(e.SALARY) FROM EMPLOYEES e  JOIN JOBS j  
